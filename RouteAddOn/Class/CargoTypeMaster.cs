...
namespace RouteAddOn.Class
{
    class CargoTypeMaster
    {
        ...
        public void ItemEvent(ref ItemEvent pVal, ref bool BubbleEvent, int formType)
        {
            try
            {
                oForm = (SAPbouiCOM.Form)ST.SBO_Application.Forms.Item("frmCt");
                    ...
                    case SAPbouiCOM.BoEventTypes.et_CLICK:
                        try
                        {
                            if (pVal.BeforeAction)
                            {
                                if (pVal.FormMode != 3)
                                {
                                    if (new[] { "tCCode", "tCName" }.Contains(pVal.ItemUID))
                                    {
        ...
        public void MenuEvent(ref MenuEvent pVal, string uniqueID, string type)
        {
            try
            {
                ...
                {
                    DisabledAllFields();
                    oForm.Items.Item("tCCode").Enabled = true;
                    oForm.Items.Item("tCCode").Click(BoCellClickType.ct_Regular);
                }
                else
                {
                    DisabledAllFields();
        ...
        private void SetCode()
        {
            try
            {
                ...
                string str = oForm.BusinessObject.GetNextSerialNumber("Code", "STCAT").ToString();
        ...
        private void DisabledAllFields()
        {
            ...
            {
                oForm.Items.Item("tCode").Enabled = false;
                oForm.Items.Item("tCCode").Enabled = false;
                oForm.Items.Item("tCName").Enabled = false;
            }
            catch (Exception e)
            {
        ...
        private bool Validation()
        {
            try
            {
                if (String.IsNullOrEmpty(((SAPbouiCOM.EditText)oForm.Items.Item("tCCode").Specific).Value))
                {
                    ST.SetText("Load Capacity Code is missing", BoStatusBarMessageType.smt_Error);
                    return false;
                }
                else if (String.IsNullOrEmpty(((SAPbouiCOM.EditText)oForm.Items.Item("tCName").Specific).Value))
                {
