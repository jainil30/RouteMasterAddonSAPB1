...
namespace RouteAddOn.Class
{
    ...
    class AllowanceMaster
    {
        ...
        public void ItemEvent(ref ItemEvent pVal, ref bool BubbleEvent, int formType)
        {
            try
            {
                oForm = (SAPbouiCOM.Form)ST.SBO_Application.Forms.Item("frmAlw");
                    ...
                    case SAPbouiCOM.BoEventTypes.et_CLICK:
                        try
                        {
                            if (pVal.BeforeAction)
                            {
                                if (pVal.FormMode != 3)
                                {
                                    if (new[] { "tACode", "tAName" }.Contains(pVal.ItemUID))
                                    {
        ...
        public void MenuEvent(ref MenuEvent pVal, string uniqueID, string type)
        {
            try
            {
                ...
                {
                    DisabledAllFields();
                    oForm.Items.Item("tACode").Enabled = true;
                    oForm.Items.Item("tACode").Click(BoCellClickType.ct_Regular);
                }
                else
                {
                    DisabledAllFields();
        ...
        private void SetCode()
        {
            try
            {
                ...
                string str = oForm.BusinessObject.GetNextSerialNumber("Code", "STALW").ToString();
        ...
        private void DisabledAllFields()
        {
            ...
            {
                oForm.Items.Item("tCode").Enabled = false;
                oForm.Items.Item("tACode").Enabled = false;
                oForm.Items.Item("tAName").Enabled = false;
            }
            catch (Exception e)
            {
        ...
        private bool Validation()
        {
            try
            {
                if (String.IsNullOrEmpty(((SAPbouiCOM.EditText)oForm.Items.Item("tACode").Specific).Value))
                {
                    ST.SetText("Allowance Code is missing", BoStatusBarMessageType.smt_Error);
                    return false;
                }
                else if (String.IsNullOrEmpty(((SAPbouiCOM.EditText)oForm.Items.Item("tAName").Specific).Value))
                {
                    ST.SetText("Allowance Name is missing", BoStatusBarMessageType.smt_Error);
                    return false;
                }
                else
                {
